...
function PortfolioTooltip({ active, payload }: any) {
  ...
  const d = payload[0].payload as MonthlyPortfolioData;
...
export default function MonthlyReturnChart({ symbol }: MonthlyReturnChartProps) {
  const [data, setData] = useState<MonthlyPortfolioData[]>([]);
      ...
      <div style={{ width: '100%', height: 200 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 4, bottom: 0, left: -8 }}>
            <defs>
              <linearGradient id={`grad-portfolio-${symbol}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              dot={false}
            />
            {/* 평가금액 라인 (실선 + gradient fill) */}
            <Area
              type="monotone"
              dataKey="portfolioValue"
              stroke={profitColor}
              strokeWidth={2}
              fill={`url(#grad-portfolio-${symbol})`}
              dot={false}
              activeDot={{
                r: 4,
                stroke: profitColor,
